      ...
      <div className={`grid ${grid} gap-6`}>
        {members.map((m) => (
          ...
          >
            ...
            <p className="mt-1 text-ink-500 text-[12px] font-[700] uppercase tracking-[0.1em]">{m.title}</p>
          </button>
        ))}
      </div>
          ...
          >
            ...
            <div className="p-8 lg:p-10">
              <button
                onClick={() => setActive(null)}
                aria-label="Close"
                className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center text-ink-500 hover:text-ink-900 transition-colors"
              >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
                  <path d="M6 6l12 12M6 18L18 6" />
                </svg>
              </button>
              <h3 className="text-ink-900 text-[24px] font-[800] leading-tight pr-8">{active.name}</h3>
              <p className="mt-1.5 text-ink-500 text-[12px] font-[700] uppercase tracking-[0.12em]">{active.title}</p>
